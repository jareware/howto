...
* Install some software: `sudo apt update && sudo apt install -y gnome-tweak-tool gpaste docker.io konsole fdupes vim gimp direnv tig curl adb ffmpeg exiftool graphicsmagick gnome-sushi python3-pip awscli kazam vlc virtualbox aha graphviz tree gthumb ffmpegthumbnailer imagemagick testdisk uuid jq geographiclib-tools net-tools qpdf usb-creator-gtk` and `sudo snap install insomnia`
